...
const checkExeStatus = async () => {
  try {
    ...
    const response = await axios.get("http://localhost:3336/check-exe-status")
...
const startExe = async () => {
  try {
    ...
    // 启动程序
    const startResponse = await axios.get("http://localhost:3336/start-exe", {
...
const updateWindowPosition = async () => {
  ...
  try {
    ...
    while (retries > 0 && !success) {
      try {
        const positionResponse = await axios.post("http://localhost:3336/position-window", position, { timeout: 5000 })
...
<template>
  <div>
    ...
    <div class="button-group">
      <button @click="startExe" :disabled="isExeRunning" class="exe-btn">
