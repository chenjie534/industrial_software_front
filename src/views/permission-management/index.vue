...
import {
  getUserPageApi,
  changePermissionApi,
  updateUserTaskPermissionApi, // 新增任务权限API
  updateUserOrganizationApi // 新增组织修改API
} from "@/api/permission"
import {
...
const showEditTaskDialog = ref(false) // 新增任务权限对话框
const showEditOrgDialog = ref(false) // 新增组织修改对话框
...
// 修改任务权限 - 新增
const updateTaskPermission = async (row: PermissionUser) => {
  try {
    await ElMessageBox.confirm(`确定要修改用户的任务权限吗？`, "任务权限修改确认", {
      confirmButtonText: "确定",
      cancelButtonText: "取消",
      type: "warning"
    })

    const newTaskPermission = row.taskPermission === 1 ? 0 : 1
    const response = await updateUserTaskPermissionApi(row.userId, newTaskPermission)

    if (response.code === 200) {
      row.taskPermission = newTaskPermission
      ElMessage.success("任务权限修改成功")
    } else {
      ElMessage.error(response.message || "任务权限修改失败")
    }
  } catch (error) {
    if ((error as Error).message !== "cancel") {
      console.error("任务权限修改失败:", error)
      ElMessage.error("任务权限修改失败")
    }
  }
}
...
<template>
  <div class="user-page">
    ...
    <el-table :data="userData" style="width: 100%" v-loading="isLoading">
      ...
      <!-- 新增任务权限列 -->
      <el-table-column prop="taskPermission" label="任务权限" width="150">
        <template #default="{ row }">
          <el-tag :type="row.taskPermission === 1 ? 'success' : 'warning'">
            {{ row.taskPermission === 1 ? "组织权限" : "个人权限" }}
          </el-tag>
        </template>
      </el-table-column>
      ...
      <el-table-column label="操作" width="320">
        <template #default="{ row }">
          ...
          <el-button size="small" type="warning" @click="updateTaskPermission(row)">修改任务权限</el-button>
    ...
    <el-dialog v-model="showEditOrgDialog" title="修改用户组织" width="500px">
      ...
      <template #footer>
        ...
        <el-button
          type="primary"
          @click="updateUserOrganization(currentUser, currentUser.orgId || '')"
    ...
    <!-- 修改任务权限对话框 - 新增 -->
    <el-dialog v-model="showEditTaskDialog" title="修改任务权限" width="500px">
      <div v-if="currentUser">
        <p>用户名: {{ currentUser.username }}</p>
        <p>
          当前任务权限:
          <el-tag :type="currentUser.taskPermission === 1 ? 'success' : 'warning'">
            {{ currentUser.taskPermission === 1 ? "组织权限" : "个人权限" }}
          </el-tag>
        </p>
        <div style="margin-top: 20px">
          <el-radio-group v-model="currentUser!.taskPermission">
            <el-radio :label="0">个人权限</el-radio>
            <el-radio :label="1" style="margin-left: 20px">组织权限</el-radio>
          </el-radio-group>
        </div>
        <div style="margin-top: 15px; color: #666; font-size: 14px">
          <p v-if="currentUser!.taskPermission === 0">
            <strong>个人权限说明：</strong>用户只能查看和管理自己创建的任务
          </p>
          <p v-else><strong>组织权限说明：</strong>用户可以查看和管理所在组织内的所有任务</p>
        </div>
      </div>
      <template #footer>
        <el-button @click="showEditTaskDialog = false">取消</el-button>
        <el-button type="primary" @click="updateTaskPermission(currentUser!)"> 确定 </el-button>
      </template>
    </el-dialog>
    <!-- 添加创建组织对话框 -->
